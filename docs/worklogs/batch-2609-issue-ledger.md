...
| #1810 remove the preset variant enum (`tenferro_tensor_core::Tensor`) | Design Gate, implemented | `DefaultScalars` became an opaque struct over a private inline payload enum: the seven preset variants are no longer public API, `pub use DefaultScalars as Tensor;` keeps the crate's public name for the value, and the measured alternatives (216 B + one `Arc` allocation for an erased payload; 24 B tag-only) were rejected in favour of the 104 B allocation-free inline shape the design record already chose. | `the_opaque_value_keeps_the_typed_access_paths`, `the_default_set_reports_every_member`, the migrated `scalar_set_import_forms.rs` and `ext/df64-proof` composition test; `cargo test -j 16 --workspace` and workspace clippy pass. Design record: `docs/worklogs/issue-1810-opaque-core-tensor.md`. |
...
- #1810 implemented: the preset variant enum became an opaque value over a private inline payload, keeping the 104 B / allocation-free representation and the `Tensor` re-export, with the measured comparison of the erased alternatives in `docs/worklogs/issue-1810-opaque-core-tensor.md`.
